...
class IPLoM:
	...
	def WriteEventToFile(self, eventFilePath):
		...
		for event in self.eventsL:
			matched_lengths = [len(x) - 2 for x in self.output if int(x[1]) == event.eventId]
			match_mean = str(mean(matched_lengths))
			writeEvent.write(str(event.eventId) + '\t' + ' '.join(event.eventStr) + '\t' + match_mean + '\n')
		writeEvent.close()
	...
	def WriteLogWithEventID(self, outputPath):
		...
		with open(outputPath + means + '.txt') as f:
